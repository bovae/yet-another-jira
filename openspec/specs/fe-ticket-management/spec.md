...
## Purpose
Frontend ticket management: list tickets with a team filter, create tickets (team, type, state, optional epic, title, body), view ticket details, and edit or delete tickets. The screens display ticket types and states as human-readable labels while sending canonical API codes, scope the epic Select to the selected team, resolve team and epic names and the current user's email, and surface RFC 9457 problem details in dialogs.
