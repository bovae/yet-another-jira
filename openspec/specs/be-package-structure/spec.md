# be-package-structure

## Purpose
Backend code is organized package-by-feature: each feature owns its vertical web slice (controller, service, DTOs), the `domain/` package is a shared kernel of JPA entities/enums/repositories, and `web/` holds only cross-cutting HTTP infrastructure. Co-located classes default to minimal visibility.
